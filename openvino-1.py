...
ie = Core()
model_path = r"C:\Users\Muhammad A. Fadillah\Desktop\python_cctv_testing\yolov8n.xml"
...
def count_objects(frame, monitoring_code):
    ...
    # Assuming results[0] contains the detections
    detections = results[0].squeeze()  # Remove any singleton dimensions
    current_vehicles = {}
    current_persons = {}
    ...
    for detection in detections:
        # Ensure detection is properly indexed
        class_id = int(detection[1].item())  # Convert tensor to scalar value
        label = None
        ...
        if class_id in [2, 3, 5, 7]:
            ...
            x1, y1, x2, y2 = map(int, detection[3:7].tolist())
            ...
            if box_x1 < vehicle_center[0] < box_x2 and box_y1 < vehicle_center[1] < box_y2:
                ...
                cv2.putText(frame, f'{label}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 0, 0), 2)

        elif class_id == 0:
            x1, y1, x2, y2 = map(int, detection[3:7].tolist())  # Convert tensor to list
            ...
            if box_x1 < person_center[0] < box_x2 and box_y1 < person_center[1] < box_y2:
                ...
                cv2.putText(frame, 'Person', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 2)
...
def generate_frames(monitoring_code):
    ...
    while True:
        ...
        vehicle_counts, total_count, persons = count_objects(frame, monitoring_code)
